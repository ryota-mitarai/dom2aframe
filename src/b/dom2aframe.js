...
AFRAME.registerComponent('auto-enter-vr', {
  init: function () {
    if (window.location.search.indexOf('stayInVR=true') !== -1) this.el.sceneEl.enterVR();
  },
});
...
class Dom2Aframe {
  constructor() {
    ...
    if (a_scenes.length > 0) {
      ...
      if (this.a_scene.hasLoaded) this.initiate();
      else this.a_scene.addEventListener('loaded', this.initiate.bind(this));
    } else {
  ...
  initiate() {
    ...
    if (!this.was_already_a_scene) document.body.appendChild(this.a_scene);
  ...
  initStyles() {
    ...
    this.vrcss.innerHTML =
      '*{--vr-x:null;--vr-y:null;--vr-z:null;--vr-scale:null;--vr-rotate:null;--vr-position:null;} .a-enter-vr{position: fixed;} .a-scene{position:fixed; top:0;} .a-html{ position: static; } .a-body{ height: initial; margin: 0px; overflow: initial; padding: initial; width: initial; }';
    document.body.appendChild(this.vrcss);

    //Style that changes when in vr
    this.invr_css =
      '.a-canvas{display: default; position: fixed; } a-scene{width: 100%; height: 100%;} *{user-select: none;}';
  ...
  elementIsInList(list, element_name) {
    for (var i = 0; i < list.length; i++) if (list[i] == element_name) return true;
  ...
  AddNewElement(element) {
    if (
      ...
      element.tagName.startsWith('A-') ||
      element.added_to_a_scene
    )
      return;
    ...
    if (
      element.innerHTML ==
      '<div classname="t" onsubmit="t" onchange="t" onfocusin="t" style="margin: 0px; border: 0px; box-sizing: content-box; width: 1px; padding: 1px; display: block; zoom: 1;"><div style="width: 5px;"></div></div>'
    )
      return;
    ...
    if (this.elementIsInList(container_elements, element.tagName))
      ...
      this.elementIsInList(text_elements, element.tagName) ||
      (typeof element.tagName == 'string' &&
        element.tagName.startsWith('H') &&
        parseFloat(element.tagName.split('H')[1]))
    ) {
      //Text based elements
  ...
  createAllElements() {
    ...
    if (this.camera.getAElement().hasLoaded) this.a_element_container.appendChild(body);
      ...
      this.camera.getAElement().addEventListener(
        'loaded',
        function () {
  ...
  showVideoPlayer() {
    ...
    if (v_element_visibility) position = { x: position.x, y: position.y + 1000, z: position.z };
    else position = { x: position.x, y: position.y - 1000, z: position.z };

  ...
  function (t, e) {
    ...
    AFRAME.registerComponent('video-controls', {
      schema: {
        ...
        infoTextBottom: { default: i },
        infoTextTop: { default: a },
      ...
      init: function () {
        ...
        (this.bar_steps = 10),
          ...
          (t.play_image_src = document.getElementById('video-play-image')
            ? '#video-play-image'
            : 'https://res.cloudinary.com/dxbh0pppv/image/upload/c_scale,h_512,q_10/v1471016296/play_wvmogo.png'),
          (t.pause_image_src = document.getElementById('video-pause-image')
            ? '#video-pause-image'
          ...
          this.video_el.paused
            ? this.play_image.setAttribute('src', t.play_image_src)
            : this.play_image.setAttribute('src', t.pause_image_src),
          ...
          this.play_image.addEventListener('click', function (e) {
            t.video_el.paused
              ? (this.setAttribute('src', t.pause_image_src), t.video_el.play())
              : (this.setAttribute('src', t.play_image_src), t.video_el.pause()),
              e.stopPropagation(),
              e.preventDefault();
          }),
          ...
          this.bar.addEventListener('click', function (e) {
            var i = document
                ...
                .components.raycaster.raycaster.intersectObject(this.object3D, !0)[0].point,
            ...
            t.position_control_from_camera(),
              this.addEventListener('dblclick', function () {
                ...
                0 == e.intersectObject(t.el.object3D, !0).length &&
                  (t.el.getAttribute('visible')
                    ? t.el.setAttribute('visible', !1)
                    : (t.el.setAttribute('visible', !0), t.position_control_from_camera()));
              });
          });
      },
      update: function (t) {
        ...
        if ('undefined' == typeof this.last_time || t - this.last_time > 250) {
          if (this.video_el.readyState > 0) {
            ...
            if (this.video_el.buffered.length > 0) {
              ...
              var n = this.context;
              if (
                ((n.fillStyle = this.data.backgroundColor),
                n.fillRect(0, 0, this.bar_canvas.width, this.bar_canvas.height),
                (n.font = this.data.timeTextFont),
                (n.fillStyle = 'white'),
                (n.textAlign = 'center'),
                n.fillText(r, this.bar_canvas.width / 2, 0.65 * this.bar_canvas.height),
                this.video_el.seeking)
              )
                (n.font = this.data.statusTextFont),
                  (n.fillStyle = this.data.textColor),
                  (n.textAlign = 'end'),
                  n.fillText('Seeking', 0.95 * this.bar_canvas.width, 0.6 * this.bar_canvas.height);
              else {
                var l = (this.video_el.buffered.end(this.video_el.buffered.length - 1) / this.video_el.duration) * 100;
                (n.font = this.data.statusTextFont),
                  (n.fillStyle = this.data.textColor),
                  (n.textAlign = 'end'),
                  n.fillText(l.toFixed(0) + '% loaded', 0.95 * this.bar_canvas.width, 0.6 * this.bar_canvas.height);
              }
              (n.fillStyle = this.data.textColor),
                (n.font = this.data.infoTextFont),
                (n.textAlign = 'center'),
                n.fillText(this.data.infoTextTop, this.bar_canvas.width / 2, 0.8 * this.bar_canvas.height),
                n.fillText(this.data.infoTextBottom, this.bar_canvas.width / 2, 0.95 * this.bar_canvas.height);
            ...
            this.bar.object3D.children.length > 0 &&
              (null === this.bar.object3D.children[0].material.map &&

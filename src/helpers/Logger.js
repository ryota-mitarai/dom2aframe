class Logger {
  ...
  ResetUpdateLoop() {
    ...
    if (this.updateLoopOutputContainer) this.updateLoopOutputContainer.innerHTML = '';
  }
  ...
  RenderUpdatedElements() {
    ...
    if (this.updateLoopOutputContainer) {
      ...
      for (let wrap of this.updatedElements) {
        ...
        let style = '';
        if (wasForced && wasDirty) style = 'padding: 5px; background-color: blue; color: white;';
        else if (wasDirty) style = 'padding: 5px; background-color: green; color: white;';
        else if (wasForced) style = 'padding: 5px; background-color: red; color: white;';
        else if (mutation) style = 'padding-left: 20px; background-color: lightgreen; color: black;';

        output +=
          '<p style="' + style + '">' + wrap.timestamp + ' : ' + identifier + ' (' + JSON.stringify(mutation) + ')</p>';
      }

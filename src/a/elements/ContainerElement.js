class ContainerElement extends Element {
  constructor(DOM2AFrame, domelement, depth, registerEvents = true) {
    ...
    if (
      ...
      (this.domelement.getAttribute('type') == 'checkbox' || this.domelement.getAttribute('type') == 'radio')
    ) {
  ...
  ElementSpecificUpdate(element_style) {
    ...
    if (backgroundImage != 'none' && isUrl(backgroundImage)) {
      ...
      if (domColor == this.DOM2AFrame.settings.transparantColor) {
        ...
        if (aColor != domColor) this.aelement.setAttribute('color', domColor);
      }
    ...
    if (isNaN(this.position.x) || isNaN(this.position.y)) console.trace('Found NaN positions!');

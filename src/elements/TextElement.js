...
class TextElement extends Element {
  constructor(DOM2AFrame, domelement, depth, registerEvents = true) {
    ...
    this.backgroundPlane = new ContainerElement(
      DOM2AFrame,
      domelement,
      depth - this.DOM2AFrame.settings.layerStepSize / 2,
      false
    ); // background shouldn't register events, only our main a-text element (otherwise the same DOMElement would get double the events)
    ...
    if (registerEvents) this.SetupEventHandlers();
  ...
  ElementSpecificUpdate(element_style) {
    ...
    if (
      ...
      this.domelement.getAttribute('type') == 'text' &&
      this.domelement.getAttribute('value')
    )
      textValue = this.domelement.value;
    else textValue = stripText(this.domelement.innerHTML);
    ...
    if (domColor == this.DOM2AFrame.settings.transparantColor) this.atext.setAttribute('visible', false);
    else {
    ...
    if (!currentFont) {
      let self = this;
      let handler = () => {
        self.atext.removeEventListener(handler);
        self._UpdateTextSize(element_style);
      };
  ...
  _UpdateTextAlignment(element_style) {
    ...
    let anchor = 'left';
    if (alignment == 'center') anchor = 'center';
    else if (alignment == 'right') anchor = 'right';
    else if (alignment == 'left') anchor = 'left';
    else if (alignment == 'start')
      // same as left if text-direction is left-to-right (which we assume for now) TODO: support right-to-left
      anchor = 'left';
    else if (alignment == 'end')
      // same as right if text-direction is left-to-right (which we assume for now) TODO: support right-to-left
      anchor = 'right';
    else {
      anchor = 'left';
    ...
    if (anchor == 'left') xyz.x -= this.position.width / 2;
    // shift to the left to comply with the text anchor
    else if (anchor == 'right') xyz.x += this.position.width / 2; // shift to the right
  ...
  _UpdateTextSize(element_style) {
    ...
    if (this.currentTextValue.length < 200) wrapPixels *= 1.075;

...
class Position {
  ...
  copyPosition(pos, elem) {
    ...
    if (this.width == 0) this.width = pos.width;
    this.height = elem.clientHeight;
    if (this.height == 0) this.height = pos.height;
...
class Element {
  constructor(domelement) {
    ...
    new MutationObserver(this.checkIfDirty.bind(this)).observe(this.domelement, {
      attributes: true,
      childList: true,
      characterData: true,
      subtree: false,
  ...
  setId() {
    if (this.domelement.hasAttribute('vr-id')) this.id = this.domelement.getAttribute('vr-id');
    else this.id = 'ge_' + ++element_id;
  ...
  addFunctionality() {
    ...
    if (this.domelement.hasAttribute('show-player')) {
      this.aelement.onclick = function () {
        dom2aframe.showVideoPlayer();
        if (this.domelement.onclick) this.domelement.onclick.call(this.domelement);
      };
      is_clickable = true;
    }
    ...
    if (this.domelement.hasAttribute('play-video')) {
      ...
      this.aelement.onclick = function () {
        dom2aframe.showNewVideo(video_id);
        if (this.domelement.onclick) this.domelement.onclick.call(this.domelement);
      };
      is_clickable = true;
    }
    ...
    if (this.domelement.hasAttribute('hover')) {
      ...
      this.aelement.onmouseenter = () => {
        this.aelement.hover();
        if (this.domelement.onmouseenter) this.domelement.onmouseenter.call(this.domelement);
      };
      this.aelement.onmouseleave = () => {
        this.aelement.stopHover();
        if (this.domelement.onmouseleave) this.domelement.onmouseleave.call(this.domelement);
      };
    ...
    if (this.domelement.hasAttribute('href')) {
      this.aelement.onclick = function () {
        window.location.href = this.domelement.getAttribute('href') + '?stayInVR=true';
        if (this.domelement.onclick) this.domelement.onclick.call(this.domelement);
      };
      is_clickable = true;
    }

    if (is_clickable) this.aelement.classList.add('clickable');
    ...
    if (this.is_body) {
      ...
      this.erd.listenTo(
        ...
        function () {
          ...
          if (this.position.width != pos.width) this.setSubtreeDirty.bind(this);
        }.bind(this)
      );
    }
  }
  ...
  setSubtreeDirty() {
    ...
    for (var i = 0, len = children.length; i < len; i++) if (children[i].setSubtreeDirty) children[i].setSubtreeDirty();
  }
  ...
  SetDragEvent(element) {
    ...
    if (element.is_body && dom_element.classList.contains('a-grabbing') && !grabbing) {
      //If we are not grabbing and the class a-grabbing appears in the body we know we started grabbing
      grabbing = true;
      return true;
    } else if (element.is_body && !dom_element.classList.contains('a-grabbing') && grabbing) {
      //If we are grabbing and the a-grabbing class is gone, we know it is over, but we still have ignore the event
      grabbing = false;
      return true;
    }
    return false;
  ...
  checkIfDirty(mutation, is_animating) {
    if (this.SetDragEvent(this) || !mutation) return;

    for (var i = 0; i < mutation.length; i++) {
      //Check if the element just stayed the same as before
      if (mutation[i].oldValue === this.domelement.getAttribute(mutation[i].attributeName)) continue;

      //Check if the elements style and possibly the style of its children changed
      if (
        mutation[i].type === 'attributes' &&
        (mutation[i].attributeName === 'class' || mutation[i].attributeName === 'style')
      )
        this.setSubtreeDirty();
      else this.setDirty();
    }
  ...
  isNotHidden(style) {
    return (
      (this.is_body || this.domelement.offsetParent !== null) &&
  ...
  getTransormation(computed_style) {
    ...
    if (x == 'null' && !this.is_separate) x = 0;
    else if (x == 'null') x = this.position.vector[0];
    else if (!this.is_separate) x *= pixels_per_meter;
    else {
    ...
    if (y == 'null' && !this.is_separate) y = 0;
    else if (y == 'null') y = this.position.vector[1];
    else if (!this.is_separate) y *= pixels_per_meter;
    else {
    ...
    if (z == 'null' && !this.is_separate) z = 0;
    else if (z == 'null') z = this.position.vector[2];
    ...
    if (scale != 'null') {
      ...
      if (scale.length == 1)
        //If the user just gave one scale
        this.transformation.setScale(scale[0], scale[0]);
      else if (scale.length == 2)
        //If the user gave a scale for x and y separately
        this.transformation.setScale(scale[0], scale[1]);
    } else this.transformation.setScale(1, 1);
    ...
    } else this.transformation.setRotate(0, 0, 0);
  ...
  updatePosition() {
    ...
    if (this.is_separate)
      //The vr position is different from the page position if the element is separate
      this.vr_position = this.transformation.position;
    else if (this.is_body) {
      //The body should not take its parent in account
  ...
  checkIfSeparate(computed_style) {
    ...
    if (is_sep != 'null') return is_sep.trim() == 'absolute';
  ...
  updateOpacity(element_style) {
    ...
    if (this.is_body || this.is_separate || typeof parent_op === 'undefined')
      //If the parent has an opacity set, then the children can get max that opacity
      new_opacity = element_style.getPropertyValue('opacity');
    else new_opacity = parent_op * element_style.getPropertyValue('opacity');
    ...
    if (this.is_transparant || !this.isNotHidden(element_style)) new_opacity = 0;
  ...
  update() {
    ...
    if (this.isDirty()) {
      ...
      if (this.is_separate != this.was_separate)
        //Check if the separate is still the same
      ...
      this.aelement.setAttribute('rotation', {
        x: this.transformation.rotation[0],
        y: this.transformation.rotation[1],
        z: this.transformation.rotation[2],
      });
...
class TextElement extends Element {
  constructor(domelement, dontAddFunc) {
    ...
    if (!dontAddFunc) this.addFunctionality();
  }
  ...
  elementSpecificUpdate(element_style) {
    ...
    if (dom2aframe.transparent != element_style.getPropertyValue('color')) {
      ...
      //If the element is transparent we just don't show it
    ...
    this.aelement.setAttribute(
      'wrap-pixels',
      this.position.width / (parseFloat(element_style.getPropertyValue('font-size')) / 52)
    );
  }
}
...
class ContainerElement extends Element {
  ...
  elementSpecificUpdate(element_style) {
    ...
    background_image = background_image.substring(
      background_image.lastIndexOf('("') + 2,
      background_image.lastIndexOf('")')
    );
    //If the background image is just "n" it means that there  was none
    if (background_image === 'n') background_image = false;

    if (dom2aframe.transparent != background_color || background_image) {
      //Check if the element is not transparent or has a background image
      if (this.isUrl(background_image)) {
        //Check if the path to the background image is a legitimate url
        ...
        this.aelement.setAttribute(
          'material',
          'alphaTest: 0.5; src: #' + asset_manager.GetAsset(background_image, 'img')
        );
      } //If the background just is a color
      else this.aelement.setAttribute('color', background_color);

...
class TestRunner {
  ...
  GetNextTestPath() {
    ...
    if (currentIndex == -1) {
      console.error(
        'TestRunner:GetNextTestPath : unknown current test path!',
        window.location.pathname,
        this.testPaths
      );
      return undefined;
    } else {
      if (currentIndex < this.testPaths.length - 1) return this.testPaths[currentIndex + 1];
  ...
  LogResult(key, value) {
    ...
    if (!this.results.has(key)) this.results.set(key, new Array());
  ...
  Start(timeout = 3000) {
    ...
    } else urlParams.currentRun = parseInt(urlParams.currentRun);
    if (!urlParams.totalRuns) urlParams.totalRuns = 10;
    else urlParams.totalRuns = parseInt(urlParams.totalRuns);
    ...
    if (urlParams.currentRun != urlParams.totalRuns) {
      ...
      setTimeout(() => {
        ...
        } else testcaseResults = resultStorage.testCases[currentTestcase];
      ...
      if (nextPath) {
        window.location.href =
          [location.protocol, '//', location.host, nextPath].join('') + '?test=true&totalRuns=' + urlParams.totalRuns;
      } else {
